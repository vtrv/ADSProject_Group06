  ...
  "RV32I_HazardsTester" should "work" in {
    test(new PipelinedRV32I("src/test/programs/BinaryFile_hazards"))
      .withAnnotations(Seq(WriteVcdAnnotation)) { dut =>
        ...
        dut.io.result.expect(9.U) // ADD x3, x1, x2 → x1 fwd dist-2(WB)=4, x2 fwd dist-1(MEM)=5 → x3=9
        dut.io.exception.expect(false.B)
        dut.clock.step(1)
        ...
        dut.io.result.expect(2031.U) // SUB x6, x4, x5 → x4 fwd dist-2(WB)=2047, x5 fwd dist-1(MEM)=16 → 2031
        dut.io.exception.expect(false.B)
        dut.clock.step(1)
        ...
        dut.io.result.expect(2022.U) // XOR x7, x6, x3 → x6 fwd dist-1(MEM)=2031, x3 fwd dist-4(regfile)=9 → 2022
        dut.io.exception.expect(false.B)
        dut.clock.step(1)

        dut.io.result.expect(2047.U) // OR x8, x6, x5 → x6 fwd dist-2(WB)=2031, x5 dist-3(regfile bypass)=16 → 2047
        dut.io.exception.expect(false.B)
        dut.clock.step(1)

        dut.io.result.expect(0.U) // AND x9, x6, x5 → x6 dist-3(regfile bypass)=2031, x5 dist-4(regfile)=16 → 0
        dut.io.exception.expect(false.B)
        dut.clock.step(1)
        ...
        dut.io.result.expect(10.U) // ADD x31, x0, x30 → x30 from same-cycle regfile bypass=10 → x31=10
        dut.io.exception.expect(false.B)
        dut.clock.step(1)
